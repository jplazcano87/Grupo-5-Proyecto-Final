...
def build_prompt(user: User, context: str):
    system_prompt = '''Eres un chatbot que recomienda películas, te llamas 'PlaIA'.
    ...
    if user.favorite_genre:
        system_prompt += f'- El género favorito del usuario es: {
            user.favorite_genre}.\n'
    if user.disliked_genre:
        system_prompt += f'- El género a evitar del usuario es: {
            user.disliked_genre}.\n'

    if context:
        system_prompt += f'Además considera el siguiente contenido: {
            context}\n'
